...
export default function Dashboard() {
  ...
  // Bumped on every pull-to-refresh and handed to each self-fetching child
  // (Feed preview, transactions ticker, trending players, articles, poll)
  // as its `key` - the cheapest way to force a real remount-and-refetch on
  // all of them without threading an imperative refresh method through
  ...
  const runBoogieBackfill = useCallback(() => {
    if (!leagueID) return;
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    try {
      await Promise.all([loadLeagueInfo(), loadPulse()]);
      runBoogieBackfill();
      setRefreshTick((t) => t + 1);
    } finally {
      setRefreshing(false);
    }
  }, [loadLeagueInfo, loadPulse, runBoogieBackfill]);
  ...
  return (
    ...
    >
      ...
      <View className="pt-7">
        <FeedPreview key={refreshTick} leagueID={leagueID} />
      </View>

      <View className="pt-7">
        <TrendingPlayers key={refreshTick} leagueID={leagueID} />
      </View>

      <View className="pt-7">
        <ArticleCarousel key={refreshTick} leagueID={leagueID} />
      </View>

      <View className="mt-7 mb-8">
        <HomePoll key={refreshTick} leagueID={leagueID} />
      </View>
    </ScrollView>
  );
}
